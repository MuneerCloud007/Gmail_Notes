...
var SGNGmailDOM = function(localJQuery){
  ...
  api.userEmail = function() {
    if(_userEmail && _userEmail.indexOf('@') !== -1)
      return _userEmail;

    var hook = $("#gb a.gb_b.gb_R,#gb a.gb_Da,#gb a.gb_Ca, #gb .gb_Na");  //adapt to new UI
    var labelText = "";

    if(!hook || !labelText || labelText.indexOf("@") === -1){
      hook = $("#gb a[aria-label*='@']"); // a robust fall back
    }
    ...
    console.log("@153", labelText, hook);
    ...
    if(labelText.indexOf("(") !== -1){
      ...
      _userEmail = labelText.split(":")[1].trim();
    }
    else
  ...
  api.isPreviewPane = function(){
    ...
    // console.log("@184, has preview pane", previewPaneFound);
  ...
  // warning: not accurate, always true now
  ...
  // warning: note accurate, always false now
  ...
  api.getInboxEmailId = function(actionData){
    ...
    var messageId = JSON.parse(actionData)[0][1][0];
  ...
  api.getCurrentMessageId = function(){
    var messageId = api.getNewGmailEmailId();
  ...
  api.getPrintPageEmail = function() {
    ...
    if (printEmailNodeStr.indexOf("<") !== -1) {
      ...
      email = printEmailNodeStr.split(":")[1].trim();
    }
    email = email.trim();
